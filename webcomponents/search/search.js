...
export function search(query, contexts_, offset, id) {
    ...
    let search_in_globule = (globules, file_explorer_id) => {
                    ...
                    if (context == "blogPosts") {
                        if (contexts.length == 0) {
                            searchBlogPosts(g, query, contexts_, indexPath, offset, MAX_RESULTS, () => {
                                search_in_globule(globules, file_explorer_id)
                            }, file_explorer_id)
                        } else {
                            searchBlogPosts(g, query, contexts_, indexPath, offset, MAX_RESULTS, () => {
    ...
    // start the search
    search_in_globule(globules, id)
}

/** 
 * Search titles...
 */
function searchTitles(globule, query, contexts, indexPath, offset, max, callback, fields, id) {
    ...
    generatePeerToken(globule, token => {
        ...
        let stream = globule.titleService.searchTitles(rqst, { domain: globule.domain, token: token })
        stream.on("data", (rsp) => {
            if (rsp.hasSummary() && !fields) {
                Backend.eventHub.publish("_display_search_results_", {"file-explorer-id": id}, true)
                Backend.eventHub.publish("__new_search_event__", { query: query, "file-explorer-id": id, summary: rsp.getSummary(), contexts: contexts, offset: offset }, true)
            } else if (rsp.hasFacets() && !fields) {
                let uuid = "_" + getUuidByString(query)
                Backend.eventHub.publish(`${uuid}_search_facets_event__`, { facets: rsp.getFacets(), "file-explorer-id": id }, true)
            } else if (rsp.hasHit()) {
                let hit = rsp.getHit()
                ...
                if (!fields) {
                    ...
                    hit.getSnippetsList().forEach(val => {
                        let uuid = "_" + getUuidByString(query)
                        Backend.eventHub.publish(`${uuid}_search_hit_event__`, { hit: hit, "file-explorer-id": id, context: indexPath.substring(indexPath.lastIndexOf("/") + 1) }, true)
                    })
                } else {
...
function searchBlogPosts(globule, query, contexts, indexPath, offset, max, callback, id) {
    ...
    generatePeerToken(globule, token => {
        let stream = globule.blogService.searchBlogPosts(rqst, { domain: globule.domain, token: token })
        stream.on("data", (rsp) => {

            if (rsp.hasSummary()) {
                Backend.eventHub.publish("_display_search_results_", {"file-explorer-id": id}, true)
                Backend.eventHub.publish("__new_search_event__", { query: query, summary: rsp.getSummary(), contexts: contexts, offset: offset, "file-explorer-id": id }, true)
